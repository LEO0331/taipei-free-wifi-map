...
await Promise.all([
    writeFile(path.join(outputDirectory, 'wifi-hotspots.json'), JSON.stringify(hotspots, null, 2)),
    writeFile(path.join(outputDirectory, 'conversion-report.json'), JSON.stringify(report, null, 2)),
]);
console.log(`Converted ${report.outputCount} records (${report.validCoordinateCount} valid coordinates, ${report.outsideTaipeiCount} outside Taipei).`);
