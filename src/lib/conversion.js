import { TAIPEI_CITY_DISTRICTS, classifyWifiHotspotType, normalizeArea, normalizeColumnName, parseCoordinate, validateWifiCoordinate, } from './wifi.js';
export const WIFI_SOURCE = '臺北市公眾區免費無線上網熱點資料(新版)';
const optional = (value) => {
    const normalized = String(value ?? '').trim();
    return normalized || undefined;
};
export function normalizeWifiRow(row) {
    return Object.fromEntries(Object.entries(row).map(([key, value]) => [normalizeColumnName(key), value.trim()]));
}
export function convertWifiRow(rawRow, index) {
    ...
    return {
        ...
        isTaipeiCity: TAIPEI_CITY_DISTRICTS.includes(areaZh),
