...
export function getHotspotText(hotspot, language) {
...
export function getNearbyHotspots(hotspots, location, radiusMeters) {
    return hotspots
        .filter((hotspot) => hotspot.coordinateStatus === 'valid' &&
        hotspot.latitude !== undefined &&
        hotspot.longitude !== undefined)
        .map((hotspot) => ({
        hotspot,
        distance: calculateDistanceMeters(location.latitude, location.longitude, hotspot.latitude, hotspot.longitude),
    }))
